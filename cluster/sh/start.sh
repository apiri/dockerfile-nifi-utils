...
if [ -n "${tls_token}" ]; then
  ...
  cd /opt/nifi/certs &&  /opt/nifi/nifi-toolkit-current/bin/tls-toolkit.sh client -t ${tls_token} -c nifi-ca
  ...
  if ! zookeepercli --servers zookeeper -c ls /nifi/leaders ; then
    ...
    # sed -i -n '59{h;n;G};p' ${NIFI_HOME}/conf/authorizers.xml
  fi
...
trap "echo Received SIGTERM, beginning shutdown..." SIGKILL SIGTERM SIGHUP SIGINT EXIT;
